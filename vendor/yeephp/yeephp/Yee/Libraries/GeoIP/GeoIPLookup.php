...
/**
 * How to use:
 * 2 databases are available: geoip (only country info) and geolitecity (city data),
 * databases are provided in the same folder as is this class.
 * 
 * $geo = new GeoIPLookup(dirname(__FILE__)."\..\geoip\GeoLiteCity.dat" ); // open the database you wish 
 * $record = $geo->geoip_record_by_addr("65.48.113.25");    pass the IP into the function  
 * $geo->geoip_close();
 * 
 * some other cool stuff:
 * $geo->geoip_country_code_by_addr("78.130.148.74"); get the 2digit ISO country code
 * $geo->geoip_country_name_by_addr("78.130.148.74"); get the full name of the country
 * $geo->geoip_region_by_addr("78.130.148.74");       get the region Name of this IP if available.... 
 * 
 * USA data and Northern Europe has most complete datasets.. so don't expect BG to be perfect :)
 * 
 * additional $geo->get_time_zone($country, $region) from ie $record will give the correct timezone for this region/country!
 * 
 * This class is a port of a set of classes provided by MaxMind -> http://dev.maxmind.com/
 */

namespace Yee\Libraries\GeoIP; 
...
class GeoIPLookup extends regionvars  
{
	// global country 
	public $flags;
	public $filehandle;
	...
	function __construct( $filename, $flags = null  ) {
	...
	private function _geoip_seek_country_v6($ipnum)
	{
	...
	private function _common_get_org($seek_org)
	{
	...
	private function _get_org_v6($ipnum)
	{
	...
	private function _get_org($ipnum)
	{
	...
	private function _get_region($ipnum)
	{
		if ($this->databaseType == GEOIP_REGION_EDITION_REV0) {
